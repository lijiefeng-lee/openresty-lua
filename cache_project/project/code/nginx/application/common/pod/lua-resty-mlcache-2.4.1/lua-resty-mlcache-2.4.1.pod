...
| C<1.15.8.1>  | :heavy_check_mark:
| C<1.15.8.2>  | :heavy_check_mark:
| E<gt>           | not tested
...
(hits) from the lua_shared_dict specified in C<shm>.
...
When called, C<get()> follows the below logic:
...
C<nil> as its first argument, and no error. The same applies to cached misses
